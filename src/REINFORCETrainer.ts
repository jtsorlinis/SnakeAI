...
import { REINFORCEOptimizer } from "./REINFORCEOptimizer";
...
import type { REINFORCETransition } from "./REINFORCEOptimizer";

export class REINFORCETrainer implements TrainerController {
  private readonly reinforce = new REINFORCEOptimizer();
  ...
  private trajectories: REINFORCETransition[][] = [];
  private reinforceUpdate = 1;
  ...
  public simulate(stepCount: number): void {
    for (let i = 0; i < stepCount; i++) {
      ...
      for (let index = 0; index < this.rolloutAgents.length; index++) {
        ...
        const action = this.reinforce.sampleAction(observation);
        const previousScore = agent.score;
        this.environment.step(agent, action);
  ...
  public getState(randomBoardCount = 0): TrainerState {
    ...
    return {
      ...
      iteration: this.reinforceUpdate,
      iterationLabel: "REINFORCE update",
      alive,
      ...
      staleIterations: Math.max(
        0,
        this.reinforceUpdate - this.bestFitnessUpdate,
      ),
  ...
  private initializeRolloutBatch(): void {
    const policy = this.reinforce.getPolicyParams();
    this.rolloutAgents = [];
    this.trajectories = [];
  ...
  private getRandomBoardAgents(randomBoardCount: number): readonly Agent[] {
    ...
    const updateChanged = this.randomBoardUpdate !== this.reinforceUpdate;
    ...
    if (updateChanged || sizeChanged) {
      ...
      this.randomBoardUpdate = this.reinforceUpdate;
    }
  ...
  private runReinforceUpdate(): void {
    ...
    if (
      this.reinforceUpdate === 1 ||
      best.fitness > this.bestEverFitness
    ) {
