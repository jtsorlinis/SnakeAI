...
import { SimpleEvolutionStrategy } from "./SimpleEvolutionStrategy";
...
export class ESTrainer implements TrainerController {
  private readonly es = new SimpleEvolutionStrategy();
  ...
  public reset(): void {
    ...
    for (let i = 0; i < POP_SIZE; i++) {
      this.population.push(this.environment.createAgent(this.es.randomPolicy()));
    }
  ...
  public getState(randomBoardCount = 0): TrainerState {
    ...
    return {
      ...
      historyLabel: "ES best fitness history",
  ...
  private evolve(): void {
    const { best, nextPolicies } = this.es.evolve(this.population);
