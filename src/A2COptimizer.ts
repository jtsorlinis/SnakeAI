import {
  A2C_ENTROPY_COEFFICIENT,
  A2C_EPOCHS,
  A2C_GAE_LAMBDA,
  A2C_GAMMA,
  A2C_MAX_GRAD_NORM,
  A2C_MINIBATCH_SIZE,
  A2C_POLICY_LEARNING_RATE,
  A2C_VALUE_COEFFICIENT,
  A2C_VALUE_LEARNING_RATE,
  HIDDEN_LAYER_UNITS,
  HH_COUNT,
  H_BIAS_COUNT,
  IH_COUNT,
...
export type A2CActionSample = {
  action: number;
  value: number;
};

export type A2CUpdateStats = {
  batchSize: number;
  policyLoss: number;
  valueLoss: number;
  entropy: number;
...
export class A2COptimizer {
  ...
  public sampleAction(observation: Float32Array): A2CActionSample {
  ...
  public train(episodes: readonly A2CTransition[][]): A2CUpdateStats {
    ...
    for (let epoch = 0; epoch < A2C_EPOCHS; epoch++) {
      this.shuffle(indices);

      for (let start = 0; start < batch.size; start += A2C_MINIBATCH_SIZE) {
        const end = Math.min(batch.size, start + A2C_MINIBATCH_SIZE);
        ...
        for (let i = start; i < end; i++) {
          ...
          miniEntropy += entropy;
          if (A2C_ENTROPY_COEFFICIENT > 0) {
            this.entropyGradient(this.probabilities, this.entropyGradientBuffer);
            for (let output = 0; output < OUTPUTS; output++) {
              this.logitsGradient[output] -=
                (A2C_ENTROPY_COEFFICIENT * this.entropyGradientBuffer[output]) /
                miniBatchSize;
            }
          }
          ...
          const dLossDValue = (A2C_VALUE_COEFFICIENT * valueError) / miniBatchSize;
          this.backwardValue(
            observation,
            this.valueHidden,
            dLossDValue,
        ...
        this.clipGradient(this.policyGradient, A2C_MAX_GRAD_NORM);
        this.clipGradient(this.valueGradient, A2C_MAX_GRAD_NORM);
  ...
  private buildBatch(episodes: readonly A2CTransition[][]): BatchData {
    ...
    for (const episode of episodes) {
      ...
      for (let t = episode.length - 1; t >= 0; t--) {
        ...
        const delta =
          transition.reward + A2C_GAMMA * nextValue * nonTerminal - transition.value;
        const advantage =
          delta +
          A2C_GAMMA * A2C_GAE_LAMBDA * nonTerminal * nextAdvantage;
