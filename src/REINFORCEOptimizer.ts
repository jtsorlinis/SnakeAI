import {
  ...
  OUTPUTS,
  POLICY_PARAM_COUNT,
  REINFORCE_ENTROPY_COEFFICIENT,
  REINFORCE_EPOCHS,
  REINFORCE_GAMMA,
  REINFORCE_MAX_GRAD_NORM,
  REINFORCE_MINIBATCH_SIZE,
  REINFORCE_POLICY_LEARNING_RATE,
  REINFORCE_VALUE_COEFFICIENT,
  REINFORCE_VALUE_LEARNING_RATE,
} from "./config";
...
const ENTROPY_FLOOR = 0.02;
const ENTROPY_DECAY_UPDATES = 40;

export type REINFORCETransition = {
...
export type REINFORCEUpdateStats = {
  batchSize: number;
  policyLoss: number;
  valueLoss: number;
  entropy: number;
...
function buildHiddenWeightOffsets(
  firstOffset: number,
  hhOffset: number,
): number[] {
...
export class REINFORCEOptimizer {
  ...
  public sampleAction(observation: Float32Array): number {
    this.forwardPolicy(observation, this.policyHidden, this.logits);
    this.logitsToProbabilities(this.logits, this.probabilities);
    return this.sampleFromProbabilities(this.probabilities);
  }

  public train(episodes: readonly REINFORCETransition[][]): REINFORCEUpdateStats {
    ...
    const entropyCoefficient = this.getEntropyCoefficient();
    const advantages = new Float32Array(batch.size);
    ...
    for (let epoch = 0; epoch < REINFORCE_EPOCHS; epoch++) {
      this.computeAdvantages(batch.observations, batch.returns, advantages);
      this.normalizeAdvantages(advantages, batch.size);
      this.shuffle(indices);

      for (let start = 0; start < batch.size; start += REINFORCE_MINIBATCH_SIZE) {
        const end = Math.min(batch.size, start + REINFORCE_MINIBATCH_SIZE);
        ...
        for (let i = start; i < end; i++) {
          ...
          const advantage = advantages[sampleIndex];
          ...
          miniPolicyLoss += -advantage * Math.log(actionProb);

          const dLossDLogProb = -advantage / miniBatchSize;
          ...
          if (entropyCoefficient > 0) {
            ...
            for (let output = 0; output < OUTPUTS; output++) {
              this.logitsGradient[output] -=
                (entropyCoefficient * this.entropyGradientBuffer[output]) /
                miniBatchSize;
            }
          }
          ...
          const dLossDValue =
            (REINFORCE_VALUE_COEFFICIENT * valueError) / miniBatchSize;
          this.backwardValue(
            observation,
            this.valueHidden,
            dLossDValue,
        ...
        this.clipGradient(this.policyGradient, REINFORCE_MAX_GRAD_NORM);
        this.clipGradient(this.valueGradient, REINFORCE_MAX_GRAD_NORM);
  ...
  private getEntropyCoefficient(): number {
    const decay = Math.exp(-this.updateCount / ENTROPY_DECAY_UPDATES);
    return REINFORCE_ENTROPY_COEFFICIENT * Math.max(ENTROPY_FLOOR, decay);
  }
  ...
  private buildBatch(episodes: readonly REINFORCETransition[][]): BatchData {
    ...
    for (const episode of episodes) {
      ...
      const episodeReturns = new Float32Array(episode.length);
      let runningReturn = 0;
      for (let t = episode.length - 1; t >= 0; t--) {
        runningReturn = episode[t].reward + REINFORCE_GAMMA * runningReturn;
        episodeReturns[t] = runningReturn;
      }
      ...
      for (let t = 0; t < episode.length; t++) {
        ...
        returns[cursor] = episodeReturns[t];
        cursor += 1;
      }
    }
  ...
  private computeAdvantages(
    ...
    for (let i = 0; i < returns.length; i++) {
      const baseline = this.forwardValue(observations[i], this.valueHidden);
      target[i] = returns[i] - baseline;
    }
  }
